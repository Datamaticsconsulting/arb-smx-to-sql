/* #############################################################################################
#  File-name   	: SGK_LOCTR_10000403.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD SGK_LOCTR
#  -------------------------------------------------------------------------
#  Purpose     	: The purpose of this file is to load data into the SGK_LOCTR
#  Type			: SCD-TYPE-1
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','SGK_LOCTR_10000403');
...
WHERE B.SESSION_NAME = 'SGK_LOCTR_10000403';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('SGK_LOCTR_10000403','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.SGK_LOCTR_10000403 ALL;
...
FROM
	(
		...
		(
			SELECT C_NAZ_INCO
			,'No Info in SMX' AS SRC_TAB_KEY,'CIF-KSA_SR00W206' AS SRC_TAB_NAME,	'C_NAZ_INCO' AS SRC_COL_NAME
			FROM &&ENV_STAGE.CIF-KSA_SR00W206 SRC 
			
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER (QUALIFY ON NATURAL_KEY AND ORDER BY CDC_TIMESTAMP)=1
	) SRC
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('SGK_LOCTR_10000403','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'SGK_LOCTR_10000403'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('SGK_LOCTR_10000403',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('SGK_LOCTR_10000403','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
