/* #############################################################################################
#  File-name   	: APLCTN_EV_10504601.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD APLCTN_EV
#  -------------------------------------------------------------------------
#  Purpose     	: The purpose of this file is to load data into the APLCTN_EV
#  Type			: SCD-TYPE-1
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','APLCTN_EV_10504601');
...
WHERE B.SESSION_NAME = 'APLCTN_EV_10504601';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('APLCTN_EV_10504601','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.APLCTN_EV_10504601 ALL;
...
INSERT INTO &&ENV_LDR.APLCTN_EV_10504601
(EV_ID,APLCTN_ID,INST_ID,RCORD_ID,REC_IND,INSRT_SESS_ID,UPDT_SESS_ID,RECYCLE_FLAG,RECYCLE_REASON,SRC_TAB_KEY,SRC_TAB_NAME,SRC_COL_NAME,RECYCLE_CDC_TIMESTAMP)

SELECT
	SGK_EV.EV_ID AS EV_ID,SGK_APLCTN.APLCTN_ID AS APLCTN_ID,INST_TYPE.INST_TYPE_CD AS INST_ID
    ,10504601 AS RCORD_ID
	,'I' AS REC_IND
	,SESS.EXECUTION_SESSION_ID 	
	,SESS.EXECUTION_SESSION_ID
	,CASE WHEN (SGK_EV.EV_ID IS NULL OR SGK_APLCTN.APLCTN_ID IS NULL OR INST_TYPE.INST_TYPE_CD IS NULL) THEN 'Y' ELSE 'N' END AS RECYCLE_FLAG,                  
	(CASE WHEN SGK_EV.EV_ID  IS NULL THEN 'EV_ID MISSED' ELSE 'EV_ID MATCHED' END||'-'||CASE WHEN SGK_APLCTN.APLCTN_ID  IS NULL THEN 'APLCTN_ID MISSED' ELSE 'APLCTN_ID MATCHED' END||'-'||CASE WHEN INST_TYPE.INST_TYPE_CD  IS NULL THEN 'INST_ID MISSED' ELSE 'INST_ID MATCHED' END) AS RECYCLE_REASON,
...
FROM
	(
		...
		(
			SELECT PG_RQS,NU_PTL,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB01' AS SRC_TAB_NAME,	'PG_RQS,NU_PTL,CDC_TIMESTAMP' AS SRC_COL_NAME
			...
			SELECT PG_RQS,NU_PTL,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB01' AS SRC_TAB_NAME,	'PG_RQS,NU_PTL,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE_RECYCLE.CTF-KSA_ICTBB01_APLCTN_EV_10504601 SRC 
			WEHRE  PG_RQS IS NOT NULL AND TRIM(PG_RQS)<> '' '
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('APLCTN_EV_10504601','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'APLCTN_EV_10504601'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('APLCTN_EV_10504601',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('APLCTN_EV_10504601','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
