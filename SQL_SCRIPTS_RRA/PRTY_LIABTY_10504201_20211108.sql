/* #############################################################################################
#  File-name   	: PRTY_LIABTY_10504201.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD PRTY_LIABTY
#  -------------------------------------------------------------------------
#  Purpose     	: The purpose of this file is to load data into the PRTY_LIABTY
#  Type			: SCD-TYPE-1
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','PRTY_LIABTY_10504201');
...
WHERE B.SESSION_NAME = 'PRTY_LIABTY_10504201';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('PRTY_LIABTY_10504201','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.PRTY_LIABTY_10504201 ALL;
...
INSERT INTO &&ENV_LDR.PRTY_LIABTY_10504201
(PRTY_ID,PRTY_LIABTY_ID,INST_ID,RCORD_ID,REC_IND,INSRT_SESS_ID,UPDT_SESS_ID,RECYCLE_FLAG,RECYCLE_REASON,SRC_TAB_KEY,SRC_TAB_NAME,SRC_COL_NAME,RECYCLE_CDC_TIMESTAMP)

SELECT
	SGK_PRTY.PRTY_ID AS PRTY_ID,SGK_LIABTY.PRTY_LIABTY_ID AS PRTY_LIABTY_ID,INST_TYPE.INST_TYPE_CD AS INST_ID
    ,10504201 AS RCORD_ID
	,'I' AS REC_IND
	,SESS.EXECUTION_SESSION_ID 	
	,SESS.EXECUTION_SESSION_ID
	,CASE WHEN (SGK_PRTY.PRTY_ID IS NULL OR SGK_LIABTY.PRTY_LIABTY_ID IS NULL OR INST_TYPE.INST_TYPE_CD IS NULL) THEN 'Y' ELSE 'N' END AS RECYCLE_FLAG,                  
	(CASE WHEN SGK_PRTY.PRTY_ID  IS NULL THEN 'PRTY_ID MISSED' ELSE 'PRTY_ID MATCHED' END||'-'||CASE WHEN SGK_LIABTY.PRTY_LIABTY_ID  IS NULL THEN 'PRTY_LIABTY_ID MISSED' ELSE 'PRTY_LIABTY_ID MATCHED' END||'-'||CASE WHEN INST_TYPE.INST_TYPE_CD  IS NULL THEN 'INST_ID MISSED' ELSE 'INST_ID MATCHED' END) AS RECYCLE_REASON,
...
FROM
	(
		...
		(
			SELECT CD_NDG_FSC,REC_CTR,ID_CREDITOR,C_TP_PROD_APPL,ACCOUNT_NUMBER,PG_RQS,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB08' AS SRC_TAB_NAME,	'CD_NDG_FSC,REC_CTR,ID_CREDITOR,C_TP_PROD_APPL,ACCOUNT_NUMBER,PG_RQS,CDC_TIMESTAMP' AS SRC_COL_NAME
			...
			SELECT CD_NDG_FSC,REC_CTR,ID_CREDITOR,C_TP_PROD_APPL,ACCOUNT_NUMBER,PG_RQS,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB08' AS SRC_TAB_NAME,	'CD_NDG_FSC,REC_CTR,ID_CREDITOR,C_TP_PROD_APPL,ACCOUNT_NUMBER,PG_RQS,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE_RECYCLE.CTF-KSA_ICTBB08_PRTY_LIABTY_10504201 SRC 
			WEHRE REC_CTR IS NOT NULL AND TRIM(REC_CTR)<> '' AND ID_CREDITOR IS NOT NULL AND TRIM(ID_CREDITOR)<> ''  AND C_TP_PROD_APPL IS NOT NULL AND TRIM(C_TP_PROD_APPL)<> '' AND ACCOUNT_NUMBER IS NOT NULL AND TRIM(ACCOUNT_NUMBER)<> '' AND PG_RQS IS NOT NULL AND TRIM(PG_RQS)<> ''   
...
LEFT JOIN &&PRD_HLP.SGK_LIABTY SGK_LIABTY ON SGK_LIABTY.NAT_KEY=TRIM(ACCOUNT_NUMBER)||'-'||TRIM(ID_CREDITOR)||'-'||TRIM(REC_CTR)||'-'||TRIM(PG_RQS)||'-'||TRIM(C_TP_PROD_APPL) AND SGK_LIABTY.TYPE_CD='CONSUMER LIABILITY' 
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('PRTY_LIABTY_10504201','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'PRTY_LIABTY_10504201'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('PRTY_LIABTY_10504201',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('PRTY_LIABTY_10504201','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
