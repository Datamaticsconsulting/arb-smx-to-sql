/* #############################################################################################
#  File-name   	: GNDR_TYPE_10504301.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD GNDR_TYPE
#  -------------------------------------------------------------------------
#  Purpose     	: The purpose of this file is to load data into the GNDR_TYPE
#  Type			: SCD-TYPE-1
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','GNDR_TYPE_10504301');
...
WHERE B.SESSION_NAME = 'GNDR_TYPE_10504301';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('GNDR_TYPE_10504301','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.GNDR_TYPE_10504301 ALL;
...
INSERT INTO &&ENV_LDR.GNDR_TYPE_10504301
(INST_ID,GNDR_TYPE_CD,GNDR_TYPE_NAME,GNDR_TYPE_DESC,RCORD_ID,REC_IND,INSRT_SESS_ID,UPDT_SESS_ID)

SELECT
	INST_TYPE.INST_TYPE_CD AS INST_ID,"GENERATE SGK FOR 'UNKNOWN'
GENERATE SGK BASED ON UNIQUE VALUE OF TRIM(C_SEX_APPL)" AS GNDR_TYPE_CD,TRIM(C_SEX_APPL) AS GNDR_TYPE_NAME,TRIM(C_SEX_APPL) AS GNDR_TYPE_DESC
...
FROM
	(
		...
		(
			SELECT C_SEX_APPL
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB05' AS SRC_TAB_NAME,	'C_SEX_APPL' AS SRC_COL_NAME
			FROM &&ENV_STAGE.CTF-KSA_ICTBB05 SRC 
			
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER (C_SEX_APPL IS NOT NULL AND TRIM(C_SEX_APPL)<> '' \
QUALIFY OVER PARTITION BY C_SEX_APPL AND ORDER BY CDC_TIMESTAMP)=1
	) SRC
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('GNDR_TYPE_10504301','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'GNDR_TYPE_10504301'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('GNDR_TYPE_10504301',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('GNDR_TYPE_10504301','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
