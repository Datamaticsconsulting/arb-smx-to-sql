/* #############################################################################################
#  File-name   	: DEMOG_VAL_10501807.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD DEMOG_VAL
#  -------------------------------------------------------------------------
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','DEMOG_VAL_10501807');
...
WHERE B.SESSION_NAME = 'DEMOG_VAL_10501807';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('DEMOG_VAL_10501807','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.DEMOG_VAL_10501807 ALL;
...
INSERT INTO &&ENV_LDR.DEMOG_VAL_10501807
...
'GENERATE SGK FOR UNIQUE VALUE OF TRIM(IM_MNT_OTH_DEDC)" AS DEMOG_VAL_CD,IM_MNT_OTH_DEDC AS DEMOG_VAL_NAME,NULL AS DEMOG_RNG_END_VAL,IM_MNT_OTH_DEDC AS DEMOG_VAL,NULL AS DEMOG_VAL_DESC_EN,NULL AS DEMOG_VAL_DESC_AR
    ,10501807 AS RCORD_ID
	,'I' AS REC_IND
...
FROM
	(
		...
		(
			SELECT IM_MNT_OTH_DEDC,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG03' AS SRC_TAB_NAME,	'IM_MNT_OTH_DEDC,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE.CTF-KSA_ICTBG03 SRC 
			WEHRE IM_MNT_OTH_DEDC IS NOT NULL AND TRIM(IM_MNT_OTH_DEDC)<> '' 
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER ( PARTITION BY IM_MNT_OTH_DEDC ORDER BY CDC_TIMESTAMP DESC)=1
	) SRC

	LEFT JOIN &&PRD_FSDM.DEMOG DEMOG ON DEMOG_NAME='OTHER MONTHLY DEDUCTION'
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('DEMOG_VAL_10501807','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'DEMOG_VAL_10501807'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('DEMOG_VAL_10501807',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('DEMOG_VAL_10501807','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
