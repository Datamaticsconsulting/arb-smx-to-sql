/* #############################################################################################
#  File-name   	: PRTY_DEMOG_10501803.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD PRTY_DEMOG
#  -------------------------------------------------------------------------
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','PRTY_DEMOG_10501803');
...
WHERE B.SESSION_NAME = 'PRTY_DEMOG_10501803';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('PRTY_DEMOG_10501803','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.PRTY_DEMOG_10501803 ALL;
...
INSERT INTO &&ENV_LDR.PRTY_DEMOG_10501803
...
SELECT
	SGK_PRTY.PRTY_ID AS PRTY_ID,DEMOG.DEMOG_CD AS DEMOG_CD,DEMOG_VAL.DEMOG_VAL_CD AS DEMOG_VAL_CD,INST_TYPE.INST_TYPE_CD AS INST_ID,COALESCE(DT_ASU,CDC_TIMESTAMP) AS PRTY_DEMOG_STRT_DTTM,CAST ('9999-12-31 00:00:00' AS TIMESTAMP(0) ) AS PRTY_DEMOG_END_DTTM,IM_END_SERV_BENF AS PRTY_DEMOG_VAL
...
FROM
	(
		...
		(
			SELECT IM_END_SERV_BENF,DT_ASU,CDC_TIMESTAMP,CD_NDG_FSC,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG03' AS SRC_TAB_NAME,	'IM_END_SERV_BENF,DT_ASU,CDC_TIMESTAMP,CD_NDG_FSC,CDC_TIMESTAMP' AS SRC_COL_NAME
			...
			SELECT IM_END_SERV_BENF,DT_ASU,CDC_TIMESTAMP,CD_NDG_FSC,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG03' AS SRC_TAB_NAME,	'IM_END_SERV_BENF,DT_ASU,CDC_TIMESTAMP,CD_NDG_FSC,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE_RECYCLE.CTF-KSA_ICTBG03_PRTY_DEMOG_10501803 SRC 
			WEHRE 'CD_NDG_FSC IS NOT NULL AND TRIM(CD_NDG_FSC)<> '' 
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER ( PARTITION BY CD_NDG_FSC,IM_END_SERV_BENF, CDC_TIMESTAMP ORDER BY CDC_TIMESTAMP DESC)=1
	) SRC
...
LEFT JOIN &&PRD_FSDM.DEMOG DEMOG ON DEMOG_NAME='END SERVICE BENEFIT'
LEFT JOIN &&PRD_FSDM.DEMOG_VAL DEMOG_VAL ON DEMOG_VAL.DEMOG_VAL=TRIM(COALESCE((SRC.IM_END_SERV_BENF),'UNKNOWN'))
AND DEMOG_VAL.DEMOG_CD=(SELECT DEMOG_TYPE_CD FROM DEMOG_TYPE WHERE DEMOG_TYPE_NAME='END SERVICE BENEFIT') 
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('PRTY_DEMOG_10501803','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'PRTY_DEMOG_10501803'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('PRTY_DEMOG_10501803',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('PRTY_DEMOG_10501803','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
