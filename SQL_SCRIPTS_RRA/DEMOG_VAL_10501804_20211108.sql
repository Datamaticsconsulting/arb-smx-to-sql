/* #############################################################################################
#  File-name   	: DEMOG_VAL_10501804.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD DEMOG_VAL
#  -------------------------------------------------------------------------
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','DEMOG_VAL_10501804');
...
WHERE B.SESSION_NAME = 'DEMOG_VAL_10501804';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('DEMOG_VAL_10501804','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.DEMOG_VAL_10501804 ALL;
...
INSERT INTO &&ENV_LDR.DEMOG_VAL_10501804
(DEMOG_CD,INST_ID,DEMOG_VAL_CD,DEMOG_VAL_NAME,DEMOG_RNG_END_VAL,DEMOG_VAL,DEMOG_VAL_DESC_EN,DEMOG_VAL_DESC_AR,RCORD_ID,REC_IND,INSRT_SESS_ID,UPDT_SESS_ID)

SELECT
	DEMOG.DEMOG_CD AS DEMOG_CD,INST_TYPE.INST_TYPE_CD AS INST_ID,"GENERATE SGK FOR 'UNKNOWN'
'GENERATE SGK FOR UNIQUE VALUE OF TRIM(IM_END_SERV_BENF)" AS DEMOG_VAL_CD,IM_END_SERV_BENF AS DEMOG_VAL_NAME,NULL AS DEMOG_RNG_END_VAL,IM_END_SERV_BENF AS DEMOG_VAL,NULL AS DEMOG_VAL_DESC_EN,NULL AS DEMOG_VAL_DESC_AR
...
FROM
	(
		...
		(
			SELECT IM_END_SERV_BENF,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG03' AS SRC_TAB_NAME,	'IM_END_SERV_BENF,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE.CTF-KSA_ICTBG03 SRC 
			WEHRE IM_END_SERV_BENF IS NOT NULL AND TRIM(IM_END_SERV_BENF)<> '' 
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER ( PARTITION BY IM_END_SERV_BENF ORDER BY CDC_TIMESTAMP DESC)=1
	) SRC
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('DEMOG_VAL_10501804','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'DEMOG_VAL_10501804'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('DEMOG_VAL_10501804',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('DEMOG_VAL_10501804','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
