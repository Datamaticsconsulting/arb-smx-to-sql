/* #############################################################################################
#  File-name   	: DEMOG_VAL_10501805.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD DEMOG_VAL
#  -------------------------------------------------------------------------
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','DEMOG_VAL_10501805');
...
WHERE B.SESSION_NAME = 'DEMOG_VAL_10501805';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('DEMOG_VAL_10501805','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.DEMOG_VAL_10501805 ALL;
...
INSERT INTO &&ENV_LDR.DEMOG_VAL_10501805
...
'GENERATE SGK FOR UNIQUE VALUE OF TRIM(IM_GR_SLR_SBK)" AS DEMOG_VAL_CD,IM_GR_SLR_SBK AS DEMOG_VAL_NAME,NULL AS DEMOG_RNG_END_VAL,IM_GR_SLR_SBK AS DEMOG_VAL,NULL AS DEMOG_VAL_DESC_EN,NULL AS DEMOG_VAL_DESC_AR
...
FROM
	(
		...
		(
			SELECT IM_GR_SLR_SBK,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG03' AS SRC_TAB_NAME,	'IM_GR_SLR_SBK,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE.CTF-KSA_ICTBG03 SRC 
			WEHRE IM_GR_SLR_SBK IS NOT NULL AND TRIM(IM_GR_SLR_SBK)<> '' 
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER ( PARTITION BY IM_GR_SLR_SBK ORDER BY CDC_TIMESTAMP DESC)=1
	) SRC
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('DEMOG_VAL_10501805','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'DEMOG_VAL_10501805'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('DEMOG_VAL_10501805',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('DEMOG_VAL_10501805','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
