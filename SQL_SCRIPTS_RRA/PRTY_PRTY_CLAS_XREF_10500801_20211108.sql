/* #############################################################################################
#  File-name   	: PRTY_PRTY_CLAS_XREF_10500801.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','PRTY_PRTY_CLAS_XREF_10500801');
...
WHERE B.SESSION_NAME = 'PRTY_PRTY_CLAS_XREF_10500801';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('PRTY_PRTY_CLAS_XREF_10500801','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.PRTY_PRTY_CLAS_XREF_10500801 ALL;
...
INSERT INTO &&ENV_LDR.PRTY_PRTY_CLAS_XREF_10500801
...
SELECT
	SGK_PRTY.PRTY_ID AS PRTY_ID,PRTY_CLAS_SCHM_TYPE.PRTY_CLAS_CD AS PRTY_CLAS_CD,PRTY_CLAS_VAL.PRTY_CLAS_CD AS PRTY_CLAS_VAL_CD,INST_TYPE.INST_TYPE_CD AS INST_ID,SGK_PRTY.PRTY_ID AS PRTY_ID,PRTY_CLAS_SCHM_TYPE.PRTY_CLAS_CD AS PRTY_CLAS_CD,PRTY_CLAS_VAL.PRTY_CLAS_CD AS PRTY_CLAS_VAL_CD,INST_TYPE.INST_TYPE_CD AS INST_ID,COALESCE(TM_AGG_UTE,CDC_TIMESTAMP) AS PRTY_CLAS_STRT_DTTM,COALESCE(TM_AGG_UTE,CDC_TIMESTAMP) AS PRTY_CLAS_STRT_DTTM,CAST ('9999-12-31 00:00:00' AS TIMESTAMP(0) ) AS PRTY_CLAS_END_DTTM
...
FROM
	(
		...
		(
			SELECT TM_AGG_UTE,CDC_TIMESTAMP,CD_NDG_FSC,CD_ST_CIV,CD_TIT_STU,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG04' AS SRC_TAB_NAME,	'TM_AGG_UTE,CDC_TIMESTAMP,CD_NDG_FSC,CD_ST_CIV,CD_TIT_STU,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE.CTF-KSA_ICTBG04 SRC 
			WEHRE CD_ST_CIV NOT NULL AND TRIM(CD_ST_CIV)<> '' AND  
			
			UNION ALL
			
			SELECT TM_AGG_UTE,CDC_TIMESTAMP,CD_NDG_FSC,CD_ST_CIV,CD_TIT_STU,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBG04' AS SRC_TAB_NAME,	'TM_AGG_UTE,CDC_TIMESTAMP,CD_NDG_FSC,CD_ST_CIV,CD_TIT_STU,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE_RECYCLE.CTF-KSA_ICTBG04_PRTY_PRTY_CLAS_XREF_10500801 SRC 
			WEHRE CD_ST_CIV NOT NULL AND TRIM(CD_ST_CIV)<> '' AND  
			
		) AA
        
		QUALIFY ROW_NUMBER() OVER ( PARTITION BY CD_NDG_FSC,CD_ST_CIV,CDC_TIMESTAMP ORDER BY CDC_TIMESTAMP DESC)=1
	) SRC
...
LEFT JOIN &&PRD_FSDM.PRTY_CLAS_SCHM_TYPE PRTY_CLAS_SCHM_TYPE ON HLP.PRTY_CLAS_SCHM_VAL_NAME='CIVIL STATUS') 

LEFT JOIN &&PRD_FSDM.PRTY_CLAS_VAL PRTY_CLAS_VAL ON HLP.PRTY_CLAS_VAL_NAME=TRIM(COALESCE(SRC.CD_ST_CIV),'UNKNOWN') 
AND PRTY_CLAS_VAL.PRTY_CLAS_CD=(SELECT PRTY_CLAS_SCHM_TYPE_CD FROM PRTY_CLAS_SCHM_TYPE WHERE PRTY_CLAS_SCHM_TYPE_NAME='CIVIL STATUS') 

LEFT JOIN &&PRD_FSDM.INST_TYPE INST_TYPE ON INST_TYPE_NAME='ALRAJHI BANK KSA'
LEFT JOIN &&PRD_HLP.SGK_PRTY SGK_PRTY ON SGK_PRTY.NAT_KEY=TRIM(SRC.CD_NDG_FSC)  
AND SGK_PRTY.TYPE_CD='CUSTOMER'
AND SGK_PRTY.INST_ID=(SELECT INST_TYPE_CD FROM INST_TYPE WHERE INST_TYPE_NAME='ALRAJHI BANK KSA')
LEFT JOIN &&PRD_FSDM.PRTY_CLAS_SCHM_TYPE PRTY_CLAS_SCHM_TYPE ON HLP.PRTY_CLAS_SCHM_VAL_NAME='EDUCATIONAL DEGREE') 

LEFT JOIN &&PRD_FSDM.PRTY_CLAS_VAL PRTY_CLAS_VAL ON HLP.PRTY_CLAS_VAL_NAME=TRIM(COALESCE(SRC.CD_TIT_STU),'UNKNOWN') 
AND PRTY_CLAS_VAL.PRTY_CLAS_CD=(SELECT PRTY_CLAS_SCHM_TYPE_CD FROM PRTY_CLAS_SCHM_TYPE WHERE PRTY_CLAS_SCHM_TYPE_NAME='EDUCATIONAL DEGREE') 
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('PRTY_PRTY_CLAS_XREF_10500801','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'PRTY_PRTY_CLAS_XREF_10500801'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('PRTY_PRTY_CLAS_XREF_10500801',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('PRTY_PRTY_CLAS_XREF_10500801','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
