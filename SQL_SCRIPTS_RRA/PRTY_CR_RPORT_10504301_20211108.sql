/* #############################################################################################
#  File-name   	: PRTY_CR_RPORT_10504301.sql
#  Part of     	: FSDM
#  Type        	: BTEQ - LDR
#  Job         	: LOAD PRTY_CR_RPORT
#  -------------------------------------------------------------------------
#  Purpose     	: The purpose of this file is to load data into the PRTY_CR_RPORT
#  Type			: SCD-TYPE-1
...
CALL &&ENV_METADATA.INFA_SESSION_INSRT('&&WORKFLOW_NAME','PRTY_CR_RPORT_10504301');
...
WHERE B.SESSION_NAME = 'PRTY_CR_RPORT_10504301';

/* #############################################################################################
# LOGGING Start Entry in STEPS_LOG table
 #############################################################################################*/

CALL &&ENV_METADATA.START_STEP('PRTY_CR_RPORT_10504301','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
...
DELETE &&ENV_LDR.PRTY_CR_RPORT_10504301 ALL;
...
INSERT INTO &&ENV_LDR.PRTY_CR_RPORT_10504301
(RPTG_PRTY_ID,OBLGR_PRTY_ID,CR_RPORT_DTTM,RQSTG_EV_ID,INST_ID,RCORD_ID,REC_IND,INSRT_SESS_ID,UPDT_SESS_ID,RECYCLE_FLAG,RECYCLE_REASON,SRC_TAB_KEY,SRC_TAB_NAME,SRC_COL_NAME,RECYCLE_CDC_TIMESTAMP)

SELECT
	SGK_PRTY.PRTY_ID AS RPTG_PRTY_ID,SGK_PRTY.PRTY_ID AS OBLGR_PRTY_ID,HLP_ICTBB08.ISSUE_DATE AS CR_RPORT_DTTM,SGK_EV.EV_ID AS RQSTG_EV_ID,INST_TYPE.INST_TYPE_CD AS INST_ID
    ,10504301 AS RCORD_ID
	,'I' AS REC_IND
	,SESS.EXECUTION_SESSION_ID 	
	,SESS.EXECUTION_SESSION_ID
	,CASE WHEN (SGK_PRTY.PRTY_ID IS NULL OR SGK_PRTY.PRTY_ID IS NULL OR HLP_ICTBB08.ISSUE_DATE IS NULL OR SGK_EV.EV_ID IS NULL OR INST_TYPE.INST_TYPE_CD IS NULL) THEN 'Y' ELSE 'N' END AS RECYCLE_FLAG,                  
	(CASE WHEN SGK_PRTY.PRTY_ID  IS NULL THEN 'RPTG_PRTY_ID MISSED' ELSE 'RPTG_PRTY_ID MATCHED' END||'-'||CASE WHEN SGK_PRTY.PRTY_ID  IS NULL THEN 'OBLGR_PRTY_ID MISSED' ELSE 'OBLGR_PRTY_ID MATCHED' END||'-'||CASE WHEN HLP_ICTBB08.ISSUE_DATE  IS NULL THEN 'CR_RPORT_DTTM MISSED' ELSE 'CR_RPORT_DTTM MATCHED' END||'-'||CASE WHEN SGK_EV.EV_ID  IS NULL THEN 'RQSTG_EV_ID MISSED' ELSE 'RQSTG_EV_ID MATCHED' END||'-'||CASE WHEN INST_TYPE.INST_TYPE_CD  IS NULL THEN 'INST_ID MISSED' ELSE 'INST_ID MATCHED' END) AS RECYCLE_REASON,
...
FROM
	(
		...
		(
			SELECT CD_NDG_FSC,PG_RQS,CD_NDG_FSC,PG_RQS,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB05' AS SRC_TAB_NAME,	'CD_NDG_FSC,PG_RQS,CD_NDG_FSC,PG_RQS,CDC_TIMESTAMP' AS SRC_COL_NAME
			...
			SELECT CD_NDG_FSC,PG_RQS,CD_NDG_FSC,PG_RQS,CDC_TIMESTAMP
			,'No Info in SMX' AS SRC_TAB_KEY,'CTF-KSA_ICTBB05' AS SRC_TAB_NAME,	'CD_NDG_FSC,PG_RQS,CD_NDG_FSC,PG_RQS,CDC_TIMESTAMP' AS SRC_COL_NAME
			FROM &&ENV_STAGE_RECYCLE.CTF-KSA_ICTBB05_PRTY_CR_RPORT_10504301 SRC 
			WEHRE CD_NDG_FSC IS NOT NULL AND TRIM(CD_NDG_FSC)<> ''  
	...
	LEFT JOIN &&PRD_HLP.HLP_ICTBB08 HLP_ICTBB08 ON HLP.PG_RQS = TRIM(SRC.CD_NDG_FSC) AND HLP.CD_NDG_FSC = TRIM(SRC.PG_RQS)
...
#############################################################################################*/
.IF ERRORCODE <> 0 THEN CALL &&ENV_METADATA.FAIL_STEP('PRTY_CR_RPORT_10504301','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID));
.QUIT ERRORCODE;
...
#  First input argument should be LDR table name like 'PRTY_CR_RPORT_10504301'
...
#############################################################################################*/
CALL &&ENV_METADATA.RECYCLE_HANDLING('PRTY_CR_RPORT_10504301',1,P_RESULT);
...
#############################################################################################*/
CALL &&ENV_METADATA.END_STEP('PRTY_CR_RPORT_10504301','&&WORKFLOW_NAME',(SELECT EXECUTION_SESSION_ID FROM MAX_EXECUTION_SESSION_ID),'SOURCE_TABLE_NAME',0,0,0,0,0,'LDR');
